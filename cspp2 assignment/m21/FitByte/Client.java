...
class FoodLog {
	...
	FoodLog(String foodName1, int quantity1, String date1, String time1) {
...
class WaterLog {
	...
	WaterLog(int waterQuantity1, String date1, String waterTime1) {
		...
		while (scan.hasNext()) {
			...
			try {
				...
				case "Food":
					...
					s.addFood(new FoodLog(items[0], Integer.parseInt(items[1]), items[2], items[3]));
					size++;
					break;
				case "Water":
					items = tokens[1].split(",");
					s.addWater(new WaterLog(Integer.parseInt(items[0]), items[1], items[2]));
					break;
